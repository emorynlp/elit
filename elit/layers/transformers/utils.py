...
def transformer_encode(transformer: PreTrainedModel,
    ...
    Args:
      transformer: 
      input_ids: 
      attention_mask:  (Default value = None)
      token_type_ids:  (Default value = None)
      token_span:  (Default value = None)
      layer_range: Note that the 0-th layer means embedding layer, so the last 3 layers of a 12-layer BERT will be (10, 13)
      max_sequence_length:  (Default value = None)
      average_subwords:  (Default value = False)
      ret_raw_hidden_states:  (Default value = False)
      transformer: PreTrainedModel: 
      layer_range: Union[int: 
...
def transformer_sliding_window(transformer: PreTrainedModel,
    ...
    if len(outputs) == 3:
        _, _, all_encoder_layers = outputs
        all_encoder_layers = torch.stack(all_encoder_layers)
    elif len(outputs) == 2:
        all_encoder_layers, _ = outputs
    else:
