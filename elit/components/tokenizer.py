...
class EnglishTokenizer(Tokenizer):
    def __init__(self):
        ...
        self.ABBREVIATION_PERIOD = read_word_set(os.path.join(
            resource_root, 'english_abbreviation_period.txt'))
        self.APOSTROPHE_FRONT = read_word_set(os.path.join(
            resource_root, 'english_apostrophe_front.txt'))
        self.MAP_CONCAT_WORD = read_concat_word_dict(os.path.join(
            resource_root, 'english_concat_words.txt'))
        self.HYPHEN_PREFIX = read_word_set(os.path.join(
            resource_root, 'english_hyphen_prefix.txt'))
        self.HYPHEN_SUFFIX = read_word_set(os.path.join(
    ...
    def tokenize(self, input_text: str, init_offset: int = 0) -> List[str]:
        """
        :param input_text: the input text.
        :param init_offset: the initial offset of the first token.
        :return: the dictionary contains ('tok' = list of tokens) and ('off' = list of offsets);
    ...
    @classmethod
    def segment(cls, tokens: List[str]) -> List[List[str]]:
        """
        :param tokens: the list of input tokens.
        :return: the list of sentences segmented from the input text.
        """

        def sentence(bidx: int, eidx: int) -> List[str]:
            return tokens[bidx:eidx]
